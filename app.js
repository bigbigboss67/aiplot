...
const Pagination = {
    state: {
        cis:      { page: 1, pageSize: 15, total: 0 },
        leads:    { page: 1, pageSize: 15, total: 0 },
        deals:    { page: 1, pageSize: 15, total: 0 },
        plots:    { page: 1, pageSize: 15, total: 0 },
        followup: { page: 1, pageSize: 15, total: 0 }
    },

    paginate(items, tableKey) {
        const s = this.state[tableKey];
        s.total = items.length;
        const start = (s.page - 1) * s.pageSize;
        return items.slice(start, start + s.pageSize);
    },

    render(tableKey) {
        const container = document.getElementById(`${tableKey}-pagination`);
        if (!container) return;
        const s = this.state[tableKey];
        ...
        let buttons = '';
        buttons += `<button ${s.page <= 1 ? 'disabled' : ''} onclick="Pagination.goTo('${tableKey}', ${s.page - 1})"><i class="fas fa-chevron-left"></i></button>`;

        // Smart page buttons (max 7 visible)
        const maxBtn = 7;
        let startPage = Math.max(1, s.page - 3);
        let endPage = Math.min(totalPages, startPage + maxBtn - 1);
        if (endPage - startPage < maxBtn - 1) startPage = Math.max(1, endPage - maxBtn + 1);

        if (startPage > 1) {
            buttons += `<button onclick="Pagination.goTo('${tableKey}', 1)">1</button>`;
            if (startPage > 2) buttons += `<button disabled>...</button>`;
        }
        for (let i = startPage; i <= endPage; i++) {
            buttons += `<button class="${i === s.page ? 'active' : ''}" onclick="Pagination.goTo('${tableKey}', ${i})">${i}</button>`;
        }
        if (endPage < totalPages) {
            if (endPage < totalPages - 1) buttons += `<button disabled>...</button>`;
            buttons += `<button onclick="Pagination.goTo('${tableKey}', ${totalPages})">${totalPages}</button>`;
        }

        buttons += `<button ${s.page >= totalPages ? 'disabled' : ''} onclick="Pagination.goTo('${tableKey}', ${s.page + 1})"><i class="fas fa-chevron-right"></i></button>`;

        container.innerHTML = `
            <span class="pagination-info">Showing ${s.total === 0 ? 0 : start}-${end} of ${s.total}</span>
            <div class="pagination-buttons">${buttons}</div>
            <div class="page-size-select">
                <span>Rows:</span>
                <select onchange="Pagination.changeSize('${tableKey}', Number(this.value))">
                    ${[10, 15, 25, 50, 100].map(n => `<option value="${n}" ${n === s.pageSize ? 'selected' : ''}>${n}</option>`).join('')}
                </select>
            </div>
        `;
    },
    ...
    goTo(tableKey, page) {
        this.state[tableKey].page = page;
        this._refresh(tableKey);
    },

    changeSize(tableKey, size) {
        this.state[tableKey].pageSize = size;
        this.state[tableKey].page = 1;
        this._refresh(tableKey);
    },

    _refresh(tableKey) {
        switch (tableKey) {
            case 'cis': UI.refreshCISTable(); break;
            case 'leads': UI.refreshLeadsTable(); break;
            case 'deals': UI.refreshDealsTable(); break;
            case 'plots': UI.refreshPlotsTable(); break;
            case 'followup': UI.refreshFollowUpTable(); break;
        }
    }
};
...
const UI = {
    ...
    refreshCISTable(data) {
        ...
        const paged = Pagination.paginate(source, 'cis');
    ...
    refreshLeadsTable(data) {
        ...
        const paged = Pagination.paginate(source, 'leads');
        const tbody = document.getElementById('leads-tbody');
        tbody.innerHTML = paged.map(lead => `
            <tr>
    ...
    refreshDealsTable(data) {
        ...
        const paged = Pagination.paginate(source, 'deals');
        const tbody = document.getElementById('deals-tbody');
        tbody.innerHTML = paged.map(deal => `
            <tr>
    ...
    refreshPlotsTable(data) {
        ...
        const paged = Pagination.paginate(source, 'plots');
        const tbody = document.getElementById('plots-tbody');
        tbody.innerHTML = paged.map(plot => `
            <tr>
    ...
    refreshFollowUpTable() {
        ...
        const allFollowUps = this.calculateFollowUps();
        const paged = Pagination.paginate(allFollowUps, 'followup');

        tbody.innerHTML = paged.map(fu => `
            <tr>
